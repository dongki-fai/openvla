...
# inputs = build_model_inputs(model, processor, dummy_image, dummy_instruction)
...
recipe = GPTQModifier(
    scheme        = "W4A16",
    targets       = "Linear",
    group_size    = 128,     # keep grouped kernels
    # outlier_split = True,    # ← NEW: no column padding, odd cols kept in fp16
    ignore        = ["lm_head"],
)

oneshot(
    model=model,            # ← just the language backbone
