...
MODEL_FAMILY = "worldvla" # Choose from "openvla", "cogact", "worldvla"
...
class DummyConfig():
    def __init__(self, model_family, pretrained_checkpoint):
        ...
        if model_family == "worldvla":
            self.history_type = "2h_1a_img_only"
            self.action_steps = 25

def build_model_inputs(cfg, image, instruction, processor, device):
    """
    Returns the kwargs dict you would normally pass directly to
    `OpenVLAForActionPrediction.forward(...)`.
    """

    inputs = get_model_inputs(cfg, image, instruction, processor)
        ...
        for record in dataset:
            ...
            try:
                ...
                for img_bytes in img_bytes_list:
                    ...
                    calibration_set.append(build_model_inputs(cfg, image, instruction, processor, device))
            ...
            try:
                ...
                for img_bytes in img_bytes_list:
                    if count >= num_samples:
                        return
                    image = decode_jpeg(img_bytes)
                    yield build_model_inputs(cfg, image, instruction, processor, device)
                    count += 1
...
if __name__ == "__main__":
    ...
    cfg = DummyConfig(model_family=MODEL_FAMILY, pretrained_checkpoint=PRETRAINED_CHECKPOINT)
    ...
    save_dir = f"{MODEL_FAMILY}-7b-{BENCHMARK_NAME}-pruned-2_4-{PRUNING_MODIFIER}-{pruned_scope}-Data-{CALIB_DATA_TYPE}"
